...
function heatLoadLabel(value: number | null | undefined) {
  if (value == null) return 'neutre / terrain plat';
  if (value < 35) return 'faible · versant frais';
  if (value > 65) return 'forte · versant chaud';
  ...
  async function saveObservation() {
    ...
    const item: Observation = {
      ...
      modelSnapshot: scoreAtObservation ? {
        ...
        heatLoadIndex: scoreAtObservation.heatLoadIndex,
        topographicAdjustmentPct: scoreAtObservation.topographicAdjustmentPct,
        ...
        <section className="sheet sheet-data" aria-modal="true">
          ...
          {!dataTarget || !dataWeather ? (
            ...
            <>
              ...
              <div className="data-section"><h3>Relief & microclimat</h3><div className="metric-row"><span>Altitude</span><b>{numberOrDash(dataTarget.elevation)} m</b></div><div className="metric-row"><span>Pente</span><b>{numberOrDash(dataTarget.slope, 1)}°</b></div><div className="metric-row"><span>Exposition</span><b>{aspectLabel(dataTarget.aspect)}</b></div><div className="metric-row"><span>Charge thermique</span><b>{dataTarget.heatLoadIndex == null ? '—' : `${dataTarget.heatLoadIndex}/100 · ${heatLoadLabel(dataTarget.heatLoadIndex)}`}</b></div><div className="metric-row"><span>Ciel visible</span><b>{dataTarget.microclimate?.skyViewPct == null ? '—' : `${dataTarget.microclimate.skyViewPct} %`}</b></div><div className="metric-row"><span>Horizon sud</span><b>{dataTarget.microclimate?.southHorizonDeg == null ? '—' : `${dataTarget.microclimate.southHorizonDeg.toFixed(1)}°`}</b></div><div className="metric-row"><span>Canopée LiDAR</span><b>{dataTarget.microclimate?.lidarAvailable ? `${dataTarget.microclimate.canopyCoverProxyPct ?? '—'} % · ${dataTarget.microclimate.canopyHeightM ?? '—'} m` : 'indisponible'}</b></div><div className="metric-row"><span>Structure forestière</span><b>{dataTarget.structureScore == null ? '—' : `${dataTarget.structureScore}/100`}</b></div><div className="metric-row"><span>Effet microclimat sur le moment</span><b>{dataTarget.topographicAdjustmentPct === 0 ? 'neutre' : `${dataTarget.topographicAdjustmentPct > 0 ? '+' : ''}${dataTarget.topographicAdjustmentPct} %`}</b></div></div>
              ...
              <p className="data-note">{isOnline ? 'Le cache est affiché immédiatement puis actualisé silencieusement. Le score « Moment » combine déficit hydrique, pente/exposition, horizon topographique et, quand le LiDAR HD IGN est disponible, hauteur et fermeture approximative de la canopée. L’horizon et la canopée servent surtout à estimer le microclimat : conservation de fraîcheur en période chaude ou, inversement, retard du réchauffement printanier.' : 'Mode hors ligne : le score utilise les dernières données locales disponibles. Les nouvelles observations modifient immédiatement la correction terrain.'}</p>
